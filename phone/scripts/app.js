...
$(document).ready(function() {
    ...
    ctxSip = {
        ...
        logItem : function(item) {
            ...
            if (callActive) {
                ...
                i += '<button class="btn btn-xs btn-danger btnHangUp" title="Hangup"><i class="fa fa-phone"></i></button>';
                i += '</div>';
            }
            i += '</div>';
        ...
        logShow : function() {
            ...
            if (calllog !== null) {
                ...
                // I deleted class 'hide' in #sip-log in index.html and then commented $('#sip-log').addClass('hide');
        ...
        logClear : function() {
            ...
            // I commented logShow() that Ui do not rendering and just deleted logitems
            // ctxSip.logShow();
        ...
        phoneCallButtonPressed : function(sessionid) {
            ...
            if (!s) {
                ...
                //---------RomanMeleshko------------------
                 $("#call_block").append( $(".foo:first-child") );
                //-----------------------------------------
    ...
    //------------RomamnMeleshko--------------------------
        ...
        $('#call_block').delegate('.pull-right .btnHoldResume', 'click', function(event) {
           var sessionid = $(this).closest('.foo').data('sessionid');
           ctxSip.phoneHoldButtonPressed(sessionid);
           return false;
        });

        $('#call_block').delegate('.pull-right .btnHangUp', 'click', function (event) {
            var sessionid = $(this).closest('.foo').data('sessionid');
            ctxSip.sipHangUp(sessionid);
        ...
        $('#call_block').delegate('.pull-right .btnTransfer', 'click', function(event) {
           var sessionid = $(this).closest('.foo').data('sessionid');
           ctxSip.sipTransfer(sessionid);
           return false;
        });

        $('#call_block').delegate('.pull-right .btnMute', 'click', function(event) {
           var sessionid = $(this).closest('.').data('sessionid');
           ctxSip.phoneMuteButtonPressed(sessionid);
           return false;
        });

        // Ending call while reload page
        window.onbeforeunload = function() {
           $(".pull-right .btnHangUp").trigger('click');
        }
   ...
   function toggle_tabs() {
        let call_page = $(".call_page");
        let last_call_page = $(".last_call_page");
        let contact_page = $(".contact_page");
        let blocks = [call_page, last_call_page, contact_page];
   ...
   function controlInput() {
      let buttons = $("#sip-dialpad > button");
      let inputElement = $("#numDisplay");
      let maxLength = 13;
   ...
   function addContact( saveNumber ) {

       const inputElement = $("#numDisplay")
       const elem_plus = $(".client-number > .fa-plus");
       const saveBlock = $(".saveNumberBlock");
       const cancelNumber = $(".cancelNumber");
       let get_number_phone = null;

       elem_plus.on('click', function() {
           get_number_phone = inputElement.val();

           if( inputElement.val() ) {
             saveBlock.toggle();
           }

           cancelNumber.on('click', () => {
               saveBlock.hide();
           });
   ...
   addContact( saveNumber );

   function saveNumber() {

       const saveBlock = $(".saveNumberBlock");
       const numDisplay = $("#numDisplay");
       const saveDisplay = $(".saveDisplay");
       const saveNumber = $(".saveNumber");
       let arr = [];

       saveNumber.on('click', function() {

          const name = $(saveDisplay).val().trim();
           const phone = $(numDisplay).val();
           if (name && phone) {
   ...
   function displayContacts() {
       ...
       contacts.forEach(contact => {
           ...
           i +=      '<p class="icon_user"><i class="fa-solid fa-user"></i></p>'
           i +=      '<p class="user_name">'+ contact.name +'</p>'
           i += '</div>'
           i += '<div class="contact_phone">'
           i +=     '<span>'+ contact.phone +'</span>'
           i += '</div>'
           i += '</div>'

           $(contacts_user).append(i);
   ...
   function deleteContact() {
       ...
       const elem = $(".deleteContact");

       let contacts = JSON.parse(localStorage.getItem('contacts'));

       $(elem).on('click', function() {

           const get_parent = $(this).parent();
           const get_name_record = $(get_parent).find(".user_name").text();

           contacts = contacts.filter((item) =>
               item.name !== get_name_record
           );
